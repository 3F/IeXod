    ...
    internal class TaskExecutionHost : ITaskExecutionHost, IDisposable
    {
        ...
        private TaskFactoryWrapper FindTaskInRegistry(IDictionary<string, string> taskIdentityParameters)
        {
            if (!_intrinsicTasks.TryGetValue(_taskName, out TaskFactoryWrapper returnClass))
            {
                ...
                if (String.Equals(returnClass.TaskFactory.TaskType.FullName, "net.r_eg.IeXod.Tasks.MSBuild", StringComparison.OrdinalIgnoreCase))
                {
                ...
                else if (String.Equals(returnClass.TaskFactory.TaskType.FullName, "net.r_eg.IeXod.Tasks.CallTarget", StringComparison.OrdinalIgnoreCase))
                {
                ...
                if (outputTargetIsItem)
                {
                    foreach (ITaskItem output in outputs)
                    {
                        ...
                        if (output != null)
                        {
                            ...
                            {
                                // The common case -- all items involved are net.r_eg.IeXod.Execution.ProjectItemInstance.TaskItems.  
                            ...
                            else
                            {
                                ...
                                {
                                    // Probably a net.r_eg.IeXod.Utilities.TaskItem.  Not quite as good, but we can still preserve escaping. 
