...
namespace net.r_eg.IeXod
{
    ...
    public sealed class MSBSdkResolver: SdkResolver
    {
        public override string Name => typeof(MSBSdkResolver).FullName;

        // Default resolver has priority 10000 and we want to go before it and leave room on either side of us. 
        public override int Priority => 8000; // original = 5000
        ...
        public MSBSdkResolver() 
            : this(Environment.GetEnvironmentVariable, VSSettings.Ambient)
        {
        }

        // Test constructor
        internal MSBSdkResolver(Func<string, string> getEnvironmentVariable, VSSettings vsSettings)
        {
        ...
        private Version GetMinimumMSBuildVersion(string netcoreSdkDir)
        {
            return s_minimumMSBuildVersions.GetOrAdd(
                ...
                dir => 
                {
                    ...
                    {
                        // 15.3 is smallest version for MSBuild that had resolver support and also
                        // greater than or equal to the version required by any 
                        // .NET Core SDK that did not have this file.
                        return new Version(); // net.r_eg.IeXod
                    }
        ...
        {
            string dotnetIeXodMSBSdkResolverDirectory =
                Path.GetDirectoryName(typeof(MSBSdkResolver).GetTypeInfo().Assembly.Location);
            string minimumVSDefinedSdkVersionFilePath =
                Path.Combine(dotnetIeXodMSBSdkResolverDirectory, "minimumVSDefinedSDKVersion");
